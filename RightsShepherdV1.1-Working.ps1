...
# Assemble and populate variables for later
    ...
    $ProtectedFileShares = Invoke-Sqlcmd2 -ServerInstance $SQLServer -Database $Database -Query "SELECT [ObjectID],[ObjectDesc],[TypeID] FROM [dbo].[ProtectedObjects] Where TypeID = 4"
...
# Gets Top Level File Share rights and sends to Add-RSMembers
foreach ($FileShare in $ProtectedFileShares)
{
    Remove-PSDrive -Name T -ErrorAction SilentlyContinue
    #Path to audit
    New-PSDrive -Name T -PSProvider FileSystem -Root $FileShare.ObjectDesc
    $AuditPSDrive = "T:\"
    $Folders = dir $AuditPSDrive | where {$_.psiscontainer -eq $true}
    $PermissionsDup = @()
    foreach ($Folder in $Folders)
    {   
	    $ACLs = get-acl ("FileSystem::" + $Folder.fullname) | ForEach-Object { $_.Access  }
	    Foreach ($ACL in $ACLs)
        {
           
            if ($ACL.IdentityReference -eq 'CREATOR OWNER' -or $acl.IdentityReference -eq 'NT AUTHORITY\SYSTEM') # Ignores CREATOR OWNER and NT AUTHORITY\SYSTEM security principles
            {
                Write-host 'Skipping' $ACL.IdentityReference 
            }
            else 
            {
                If ($ACL.AccessControlType -eq 'Allow')
                {
                    #$PermissionsDup.samaccountname += $ACL.IdentityReference
                    $ACLProp = @{}
                    $ACLProp.samaccountname = $ACL.IdentityReference
                    $ACLObj = New-Object –TypeName PSObject –Prop $ACLProp
                    $PermissionsDup += $ACLObj
                }       
            }
	    }
    }
    $Members = New-Object System.Collections.Generic.List[System.Object]
    #$Members += $PermissionsDup | Select-Object -Property samaccountname -Unique
    $PermissionsUnique = $PermissionsDup | Select-Object -Property samaccountname -Unique
    Foreach ($member in $PermissionsUnique)
    {
        $Members.Add($Member.samaccountname.ToString())
    }
  
    Add-RSMembers -MemberArray $members -ObjectID $FileShare.ObjectID -SQLServer $SQLServer -Database $Database
    $LastCheckSQL = "update ProtectedObjects set LastChecked=Getdate() where ObjectID = " + $FileShare.ObjectID
    Invoke-Sqlcmd2 -ServerInstance $SqlServer -Database $Database -Query $LastCheckSQL
}
